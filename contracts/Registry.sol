...
contract BRegistry {
    ...
    function getPoolsWithLimit(
        ...
        bytes32 key = _createKey(fromToken, destToken);
        result = new address[](
            Math.min(limit, _pools[key].pools.values.length - offset)
        );
    ...
    function sortPools(address[] calldata tokens, uint256 lengthLimit)
        external
    {
        for (uint256 i = 0; i < tokens.length; i++) {
            for (uint256 j = i + 1; j < tokens.length; j++) {
                bytes32 key = _createKey(tokens[i], tokens[j]);
                address[] memory pools = getPoolsWithLimit(
                    tokens[i],
                    tokens[j],
                    0,
                 ...
                 = _getEffectiveLiquidityForPools(tokens[i], tokens[j], pools);
    ...
    function sortPoolsWithPurge(address[] calldata tokens, uint256 lengthLimit)
        external
    {
        for (uint256 i = 0; i < tokens.length; i++) {
            for (uint256 j = i + 1; j < tokens.length; j++) {
    ...
    function _getEffectiveLiquidityForPoolsPurge(
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            if (token1 < token2) {
                ...
                _infos[pools[i]][key].liq = _infos[pools[i]][key].liq.mul(
                    IBPool(pools[i]).getBalance(token2)
                );
        ...
        for (uint256 i = 0; i < _pools[key].pools.length(); i++) {
            effectiveLiquidity[i] = _infos[_pools[key].pools.values[i]][key]
                .liq;
        }
    }
