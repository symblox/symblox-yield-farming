...
contract Governor {
    ...
    string public constant name = "Symblox Governor";
    ...
    /// @notice The address of the Symblox governance token
    SyxInterface public syx;
    ...
    constructor(address timelock_, address syx_, address guardian_) public {
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(syx.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "Governor::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || syx.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "Governor::cancel: proposer above threshold");
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        ...
        uint96 votes = syx.getPriorVotes(voter, proposal.startBlock);
